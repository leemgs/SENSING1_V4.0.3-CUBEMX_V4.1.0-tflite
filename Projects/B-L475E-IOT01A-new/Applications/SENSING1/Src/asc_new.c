...
#define AI_NET_OBJ_INSTANCE g_asc_logmel
 
#undef AI_ASC_LOGMEL_MODEL_SIGNATURE
#define AI_ASC_LOGMEL_MODEL_SIGNATURE     "712e5eac04a4f8a7cd1af58c9829881c"
...
#define AI_TOOLS_DATE_TIME   "Sun Sep 25 00:09:27 2022"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_ASC_LOGMEL_N_BATCHES
#define AI_ASC_LOGMEL_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 17408, 1,
                     NULL),
  AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_ASC_LOGMEL_IN_NUM, &input_0_output),
  AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_ASC_LOGMEL_OUT_NUM, &dense_2_nl_output),
  &conv2d_1_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool asc_logmel_configure_activations(
...
AI_DECLARE_STATIC
ai_bool asc_logmel_configure_weights(
...
AI_API_ENTRY
ai_bool ai_asc_logmel_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_ASC_LOGMEL_MODEL_NAME,
      .model_signature   = AI_ASC_LOGMEL_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_asc_logmel_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_asc_logmel_create(
...
AI_API_ENTRY
ai_handle ai_asc_logmel_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_asc_logmel_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= asc_logmel_configure_weights(net_ctx, &params->params);
  ok &= asc_logmel_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_asc_logmel_forward(ai_handle network, const ai_buffer* input)
{
